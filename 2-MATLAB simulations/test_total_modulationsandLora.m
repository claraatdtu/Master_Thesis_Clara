...
% AUTHOR: Clara SORRE
% This MATLAB code gathers all 3 MATLAB scripts: test_lora_only.m,
% test_mfsk_only.m, test_psk_only.m
% A plot gathers those graphs to be able to compare directly the modulation
% schemes in terms of BER vs Eb/N0.
% The final plot is displayed in the report of the Thesis but in the
% Appendix.
% IN INPUT: msg_bits_length = the total number of random input bits to simulate
% IN OUTPUT: the BER vs Eb/N0 plot (simulated graphs for each modulation scheme)
...
function test_total_modulationsandLora(msg_bits_length)
    ...
    for idx = 1:length(ebn0_range) %the BER loop for each E_b/N_0 point
        ebno = ebn0_range(idx);
        %% BPSK detection: from test_psk_only.m for BPSK and QPSK
        ...
        sp1 = sp1(:); % make sure sp1 and sp0 are column vectors
        sp0 = sp0(:);
        corr_sp1 = sum(bpskn_matrix .* sp1, 1); % correlate with sp1 and sp0
        ...
        %% FSK simulations (2-, 4-, 8-, 16-FSK): from test_mfsk_only.m
        for M = [2, 4, 8, 16] %loop for every MFSK configuration
            ...
            BER_val = mean(DcfM ~= bM); % calculate the BER
            switch M
                case 2
                    BER_F2(idx) = BER_val;
                case 4
    ...
    %% LoRa Simulation: from test_lora_only.m
    ...
    %% plot results the results
    figure;
    semilogy(ebn0_range, BER_F2, 'r-', ebn0_range, BER_F4, 'b-', ebn0_range, BER_F8, 'g-',  ebn0_range, BER_F16, 'c-', ebn0_range, BER_B, 'k--', ebn0_range, BER_Q, 'm--', ebn0_range, ber_matrix(1,:), 'o--',ebn0_range, ber_matrix(6,:), 'x--', 'LineWidth', 1)
    grid on;
    xlabel('E_b/N_0 (dB)');
