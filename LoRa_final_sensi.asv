...
% Compute z from Pb
z_values = sqrt(2) * erfcinv(4 * Pb_values);
...
for idx = 1:length(SF_values)
    ...
    SNR_linear = ((z_values + 1.28 * sqrt(SF) - 0.4) / 1.28).^2 .* (4 / (4 + p)) ./ (2.^SF);
    
    % Convert to dB
    SNR_dB = 10 * log10(SNR_linear);
    
    % Plot SNR vs. z
    plot(z_values, SNR_dB, 'Color', colors(idx, :), 'LineWidth', 1.5, 'DisplayName', sprintf('SF = %d', SF));
end
...
for idx = 1:length(p_values)
    ...
    % Plot SNR vs. z
    plot(z_values, SNR_dB, 'Color', colors(idx, :), 'LineWidth', 1.5, 'DisplayName', sprintf('p = %d', p));
end
...
for idx = 1:length(SF_values)
    ...
    % Plot SNR vs. z
    plot(Pb_values, SNR_dB, 'Color', colors(idx, :), 'LineWidth', 1.5, 'DisplayName', sprintf('SF = %d', SF));
end
...
for idx = 1:length(p_values)
    ...
    % Plot SNR vs. z
    plot(Pb_values, SNR_dB, 'Color', colors(idx, :), 'LineWidth', 1.5, 'DisplayName', sprintf('p = %d', p));
end
...
Pb_values = linspace(10e-10, 10e-6  , 1000000);  %0.25
...
for idx = 1:length(SF_values)
    ...
    % Plot SNR vs. z
    plot(Pb_values, Sensitivity_dBm, 'Color', colors(idx, :), 'LineWidth', 1.5, 'DisplayName', sprintf('SF = %d', SF));
end
...
Pb_values = linspace(10e-10, 10e-6, 1000000); % Probability of bit error
...
for idx = 1:length(p_values)
    ...
    plot(Pb_values, Sensitivity_dBm, 'Color', colors(idx, :), 'LineWidth', 1.5, 'DisplayName', sprintf('p = %d', p));
end
