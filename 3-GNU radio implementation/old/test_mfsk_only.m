...
function test_mfsk_only(msg_bits_length)
    ...
    for idx = 1:length(M_values) %loop through each M order
        ...
        semilogy(ebn0_range, BER,  '--', 'DisplayName', [num2str(M) '-FSK Sim'], 'Color', colors(idx) , 'LineWidth', 1.5); % Plot the simulated BER
        hold on;
    end
    for idx = 1:length(M_values) % Theoretical BER (non-coherent orthogonal MFSK)
        ...
        semilogy(ebn0_range, Pb, 'LineWidth', 1.5,'Marker', marker_list{idx}, 'DisplayName', [num2str(M) '-FSK Theory'], 'Color', colors(idx));%plot ber
    end
