...
% AUTHOR: Clara SORRE
% This MATLAB code enables to visualize the BER versus Eb/N0 for the
% different modulation schemes.
%It uses BER data obtained experimentally with the HackRF Ones and the GNU
%Radio. Finally, an interpolation is performed to determine eb_n0_dB at
%BER=10^(-3).
...
% BER results measured with gnu radio
ber_lorasf7 = [0.396657, 0.229680, 0.005685, 0.00002, 0.00000002];   %when ber=0: artificially 0.0000002
ber_bfsk = [0.42288, 0.34419, 0.15655, 0.01876, 0.00004];  
ber_4fsk = [0.39636, 0.29288, 0.1293, 0.01482, 0.00004];   
ber_8fsk = [0.315653, 0.201418, 0.066603, 0.00315, 0.0000002]; %when ber=0: artificially 0.0000002
...
schemes = {'LoRa SF=7', ber_lorasf7;'LoRa SF=12', ber_lorasf12; 'BFSK', ber_bfsk;'4-FSK', ber_4fsk; '8-FSK', ber_8fsk; '16-FSK', ber_16fsk;'QPSK', ber_qpsk};%store the schemes in an array
fprintf('The estimated E_b/N_0 for BER = %.1e:\n is', target_ber);
for i = 1:size(schemes,1)
    name = schemes{i,1};
    ber = schemes{i,2};
    estim = interpolate_target_ber(eb_n0_dB, ber, target_ber); %estimate the Eb/N0 for the target BER
    fprintf('%s: %.2f dB\n', name, estim);
end
% plot
figure;
